...
router.post("/events/seedDB", async (req, res) => {
  axios
    .get(
      "https://app.ticketmaster.com/discovery/v2/events.json?size=200&segment=Music&countryCode=ES&apikey=ntGPVQaujf56CfGVAEXhDAioEwQA5Apr"
    )
    .then(async response => {
      ...
      for await (event of events) {
        const newEvent = new Event({
          ...
          tickets: {
            sold: Math.floor(Math.random() * (20 - 0 + 1)) + 0,
            available: Math.floor(Math.random() * (200 - 20 + 1)) + 20
          }
        });
...
router.get("/events", async (req, res) => {
  ...
  for await (artistFilter of uniqueEvents) {
    filteredEvents.push(
      await Event.findOne({ "artist.name": artistFilter }, {})
    );
  }
